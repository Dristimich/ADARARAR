...
overlay.BackgroundTransparency = 1 -- Для плавной анимации
overlay.Text = ""
...
modal.Size = UDim2.new(0, 480, 0, 260) -- Чуть выше для правильных пропорций
modal.Position = UDim2.new(0.5, -240, 0.5, -130)
modal.BackgroundColor3 = Color3.fromRGB(33, 35, 41)
modal.Visible = false
modal.ZIndex = 2
modal.Parent = gui
...
-- КОНТЕЙНЕР БАЛАНСА (Справа вверху)
local topBalanceFrame = Instance.new("Frame")
topBalanceFrame.Size = UDim2.new(0, 200, 0, 30)
topBalanceFrame.Position = UDim2.new(1, -255, 0, 20)
topBalanceFrame.BackgroundTransparency = 1
topBalanceFrame.ZIndex = 5
topBalanceFrame.Parent = modal

local topBalanceLayout = Instance.new("UIListLayout")
topBalanceLayout.FillDirection = Enum.FillDirection.Horizontal
topBalanceLayout.HorizontalAlignment = Enum.HorizontalAlignment.Right
topBalanceLayout.VerticalAlignment = Enum.VerticalAlignment.Center
topBalanceLayout.Padding = UDim.new(0, 6)
topBalanceLayout.Parent = topBalanceFrame
...
balanceText.Parent = topBalanceFrame
...
balanceIcon.Parent = topBalanceFrame
balanceIcon.LayoutOrder = -1 -- Иконка всегда слева от текста
...
-- ЦЕНТР: ЦЕНА (Иконка + Текст)
local centerPriceFrame = Instance.new("Frame", promptContainer)
centerPriceFrame.Size = UDim2.new(1, 0, 0, 30)
centerPriceFrame.Position = UDim2.new(0, 0, 0, 115)
centerPriceFrame.BackgroundTransparency = 1
centerPriceFrame.ZIndex = 5

local centerPriceLayout = Instance.new("UIListLayout", centerPriceFrame)
centerPriceLayout.FillDirection = Enum.FillDirection.Horizontal
centerPriceLayout.HorizontalAlignment = Enum.HorizontalAlignment.Center
centerPriceLayout.VerticalAlignment = Enum.VerticalAlignment.Center
centerPriceLayout.Padding = UDim.new(0, 8)

local priceIcon = Instance.new("ImageLabel", centerPriceFrame)
priceIcon.Size = UDim2.new(0, 24, 0, 24)
priceIcon.BackgroundTransparency = 1
priceIcon.Image = "rbxassetid://70493384532723"
priceIcon.ZIndex = 10

local itemPrice = Instance.new("TextLabel", centerPriceFrame)
...
buyBtnBase.Size = UDim2.new(1, -60, 0, 50)
buyBtnBase.Position = UDim2.new(0, 30, 1, -75)
buyBtnBase.Text = ""
...
-- Полоска загрузки внутри кнопки
local progressFill = Instance.new("Frame", buyBtnBase)
progressFill.Size = UDim2.new(0, 0, 1, 0)
progressFill.Position = UDim2.new(0, 0, 0, 0)
progressFill.BackgroundColor3 = Color3.fromRGB(36, 59, 146)
...
successMsg.Position = UDim2.new(0, 20, 0, 140)
...
okBtn.Size = UDim2.new(1, -60, 0, 50)
okBtn.Position = UDim2.new(0, 30, 1, -75)
okBtn.Text = "OK"
...
-- 3. АНИМАЦИИ КНОПОК
-- ==========================================
local function applyButtonAnimations(btn)
	local originalScale = UDim2.new(1, -60, 0, 50)
	local pressedScale = UDim2.new(1, -64, 0, 48)

	btn.MouseEnter:Connect(function()
		TweenService:Create(btn, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(69, 109, 255)}):Play()
	end)
	btn.MouseLeave:Connect(function()
		TweenService:Create(btn, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(59, 99, 246)}):Play()
		TweenService:Create(btn, TweenInfo.new(0.1), {Size = originalScale}):Play()
	end)
	btn.MouseButton1Down:Connect(function()
		TweenService:Create(btn, TweenInfo.new(0.1), {Size = pressedScale, BackgroundColor3 = Color3.fromRGB(36, 59, 146)}):Play()
	end)
	btn.MouseButton1Up:Connect(function()
		TweenService:Create(btn, TweenInfo.new(0.1), {Size = originalScale, BackgroundColor3 = Color3.fromRGB(69, 109, 255)}):Play()
	end)
end

applyButtonAnimations(buyBtnBase)
applyButtonAnimations(okBtn)

-- Анимация крестика
closeBtn.MouseEnter:Connect(function()
	TweenService:Create(closeBtn, TweenInfo.new(0.2), {ImageColor3 = Color3.fromRGB(200, 200, 200)}):Play()
end)
...
local function ShowModal()
	...
	TweenService:Create(modalScale, TweenInfo.new(0.4, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Scale = 1}):Play()
end

local function HideModal()
	local t1 = TweenService:Create(overlay, TweenInfo.new(0.2), {BackgroundTransparency = 1})
	local t2 = TweenService:Create(modalScale, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {Scale = 0.8})
	t1:Play()
	t2:Play()
	t2.Completed:Wait()
	modal.Visible = false
