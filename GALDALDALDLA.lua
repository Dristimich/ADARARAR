...
setupTitle.Position = UDim2.new(0, 0, 0, 0)
...
makeLabel(setupFrame, "Set coins to value after purchase", 180)
local setInput = makeInput(setupFrame, amountToSet, "Amount to SET (0 = disabled)", 198)
...
mc.MaxSize = Vector2.new(435,185)
mc.MinSize = Vector2.new(435,185)
mc.Parent = modal
...
-- ===============================================
-- [НОВОЕ] Создаём уведомление полностью с нуля
--         по точным параметрам со скриншотов
-- ===============================================
local function showGameNotification()
    task.spawn(function()
        ...
        notifLabel.TextStrokeTransparency = 1           -- из скриншота
        ...
        -- UIStroke с зелёным цветом [0, 177, 0]
        ...
        -- Висит 5 секунд, потом удаляется
        task.wait(5)
...
buyBtn.MouseButton1Click:Connect(function()
    ...
    task.spawn(function()
        local ok, err = pcall(function()
            ...
            if setVal ~= 0 then
                current = setVal          -- устанавливаем
            end
            current = current + addVal    -- прибавляем
